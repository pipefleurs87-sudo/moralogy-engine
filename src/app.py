...
# Parche temporal para Python 3.13 + Pydantic compatibility
if sys.version_info >= (3, 13):
    os.environ["PROTOCOL_BUFFERS_PYTHON_IMPLEMENTATION"] = "python"
    import warnings
    warnings.filterwarnings('ignore', category=DeprecationWarning)

# ... resto de tu código aquíimport sys
if sys.version_info >= (3, 13):
    import warnings
    warnings.filterwarnings('ignore', category=DeprecationWarning)import streamlit as st
...
Respond ONLY in JSON: {{"can_resolve": bool, "resolution": str or null, "reason": str or null}}"""
                ...
                with col1:
                    st.markdown("**🟢 Noble Engine (Deontological)**")
                    st.info(noble)
                with col2:
                    st.markdown("**🔴 Adversary Engine (Consequentialist)**")
                    st.error(adversary)
                ...
                if resolution:
                    ...
                    st.warning(f"""**⚠️ Unresolved Dilemma**
...
3. **Ahora sí**, haz **Commit changes**

## 📝 **Paso 3: Actualiza requirements.txt**
